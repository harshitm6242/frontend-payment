...
const SendMoney = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Fetched userdfsgs:', usersData);
        setUsers(usersData);
      }
    } catch (error) {
  ...
  const fetchRecentTransactions = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8085/api/transactions/user/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const fetchUserRewards = async (userId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:8085/api/rewards/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        senderName: parseInt(id),
        receiverName: parseInt(formData.receiverId),
  ...
  const getUserName = (userId) => {
    ...
    console.log('Finding user for ID:', userId, 'Found user:', user);
        ...
        <form onSubmit={handleSubmit} className="send-money-form">
          <div className="form-group">
            ...
            <select
              ...
              required
            >
              <option value="" key="default">Select a user</option>
              {users.map(user => (
