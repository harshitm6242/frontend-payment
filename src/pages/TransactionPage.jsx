...
const Transactions = () => {
  ...
  const parseDate = (ts) => new Date(ts?.slice(0, 23));
  ...
  const handleExport = (format) => {
    ...
    if (format === 'csv') {
      ...
      link.href = url;
      link.download = `transactions-${parseDate().toISOString().split('T')[0]}.csv`;
      link.click();
  ...
  const getQuickInsights = () => {
    ...
    const currentMonth = parseDate().getMonth();
    const currentYear = parseDate().getFullYear();
    
    const thisMonthSpent = transactions
      .filter(t => {
        const date = parseDate(t.timeStamp);
